...
```{r}
#| label: data-vn
data(vndata)      # dataset, matrix time series with agg cols of cross-sectional dims
y <- vndata[, "GBDOth"] # select single cross-sectional series
...
hres_list <- lapply(model, function(fit)
...
mres <- res2matrix(hres, agg_order = m)
  # k* is calculated from the factors
  # m = 12, k* = 1 + 2 + 3 + 4 + 6 = 16, N = 228/12 = 19
  # N(k* + m) = 19(18) = 360-18 = 342
