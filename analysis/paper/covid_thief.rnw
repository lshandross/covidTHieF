...
Two models created and run by the U.S. COVID-19 Forecast Hub---the COVIDhub-baseline and COVIDhub-4\_week\_ensemble---served as benchmark comparison models for both the training and testing phases. Such benchmarks are useful when evaluating forecasting models, especially when the chosen scoring metrics are scale-dependent and thus require relative comparison among models forecasting the same targets\cite{cramer}. Forecasts for these models were queried from the Zoltar forecast archive, a research data repository developed by Reich Lab, through the \textit{covidHubUtils} package\cite{hub_data_research, zoltar}.

The COVIDhub-baseline is a naive baseline model whose forecasts are made using the previous day's incident hospitalization value as the point forecast (and 0.5 quantile for the probabilistic forecast) and past changes in daily incidence to calibrate uncertainty in its probabilistic forecasts\cite{cramer}. Beating the baseline was typically seen as one marker of a useful model. The COVIDhub-4\_week\_ensemble is an equally-weighted median ensemble of eligible\footnote{For more information about forecast eligibility, see \cite{ensembles_info}} submitting models for incident hospitalizations at the COVID-19 Forecast Hub that makes short-term forecasts for 1- to 28 days ahead. (This is the same as the main COVIDhub-ensemble, except its forecasts for horizons beyond 14-days ahead were removed due to declining-accuracy concerns\cite{ensembles_info, short_horizons}.)
...
We can show that the new notation is consistent with the old notation using our example. The first observation ($z=1)$ at the $k = 7$-day level of the second 14-day period ($i = 2$) is written in the new notation as $y_{M_7(2-1)+1}^{[7]} = y_{2(2-1)+1}^{[7]}$. This simplifies down to $y_{2+1}^{[7]} = y_3^{[7]}$ (the third overall observation in the 7-day series), which matches the original notation for $j = 3$.
...
Figure \ref{fig:figure_forecasts_plot1} shows 1- to 28-day ahead forecasts during the testing phase for a top performing model (COVIDhub-4\_week\_ensemble), poor performing model, and the two benchmark models. The COVIDhub-4\_week\_ensemble followed the true confirmed hospitalizations better than the other three models, especially during the Omicron wave’s period most rapid increase (Figure \ref{fig:figure_forecasts_plot1}). On the other hand, the COVIDhub-baseline generally had the widest 95\% interval forecasts of the plotted models. These results are mirrored in the subsequent stratified evaluations.
...
Model rankings were consistent between the average states and U.S. national scales for the overall testing phase evaluation, with the top six models outperforming the COVIDhub-baseline (Tables \ref{tab:table_overall_states} and \ref{tab:table_overall_us}). The COVIDhub-4\_week\_ensemble has the lowest WIS and MAE by a substantial margin, followed by THieF\_6wk-4root, THieF\_12wk-4root, THieF\_ensemble-train3, THieF\_ensemble-mean. The two worst-performing models---arima\_s1-noTransform and sarima\_s7-noTransform---were consistent between geographic scales. However, coverage rates differed for the two groupings, with models tending to display better coverage for the averaged states scale compared to the U.S. national one (Tables \ref{tab:table_overall_states} and \ref{tab:table_overall_us}).
...
For the by-location evaluation the top six models had better or equivalent performance than the COVIDhub-baseline (Figure \ref{fig:figure_location_plot}). The COVIDhub-4\_week\_ensemble was a clear first place for the evaluation while the other top models generally demonstrated close WIS values across all locations. THieF\_6wk-4root (second place) was marginally better than the three models ranked below it with lower relative WIS by a difference of more than 0.05 in over 18\% of locations, and THieF\_12wk-4root (ranked third) had several locations for which it outperformed the COVIDhub-4\_week\_ensemble and THieF\_6wk-4root Figure \ref{fig:figure_location_plot}. Additionally, even the lowest ranking models displayed better performance compared to the COVIDhub-baseline for many location, particularly for locations with low-cumulative hospitalization counts. This trend is perhaps unsurprising given that high counts settings tend to be more difficult to forecast accurately
