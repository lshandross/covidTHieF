...
\begin{itemize}
\item \textbf{Best (THieF) model overall:} \newline Best original THieF over the entire validation phase
\item \textbf{Does ensembling THieF improve performance:} \newline Best trained THieF ensemble over the entire validation phase
\item \textbf{Does THieF improve upon performance compared to its base forecaster:} \newline Best ARIMA/SARIMA over the entire validation phase
\item \textbf{Can weighted ensembles improve upon performance over unweighted ones:} \newline Untrained THieF ensemble
\item \textbf{Do some THieF models improve with more input data:} Best original THieF during the last 17 weeks of the validation phase
\end{itemize}
where ``best" was defined by lowest WIS. These criteria involved passing on a representative from each of the classes of models---original THieF, THieF (trained) ensemble, ARIMA/SARIMA---plus the equally weighted THieF ensemble and an additional original THieF model based on performance during the last 17 weeks of the validation phase. We limited the last category to original THieF models because we suspected such models with large aggregation levels might fit this description, which excluded the other classes of models. Thus, a maximum of five models could make up the best subset to move onto the training phase evaluations, if unique candidates occupied each category. However, if the same model occupied multiple categories, we might pass on only four models.

\subsection{Testing phase} \label{testing_eval}
Using the subset of best candidate models found during the validation phase, we made new testing phase forecasts and evaluated these models against each other and against the two comparison models from the Forecast Hub, COVIDhub-baseline and COVIDhub-4\_week\_ensemble. Like with the validation phase, all testing phase evaluations were restricted to forecasts with target end dates within the testing phase (November 1, 2021 to October 2, 2022).

The testing phase analysis was broken down into five smaller evaluations, based on aggregating the testing phase forecasts for the overall period, by horizon week, by pandemic wave, by location, and by forecast date. The smaller evaluations were intended to help pinpoint any potential accuracy gains (or losses) the original THieF or THieF ensembles had over the COVIDhub-baseline (or COVIDhub-4\_week\_ensemble) and allow us to hypothesize the origin of these accuracy differences.
