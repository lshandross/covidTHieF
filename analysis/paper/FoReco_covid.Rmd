...
```{r}
start_period <- 4
end_period <- start_period + as.numeric(end_date - start_date + 1) / freq
...
long_ts <- ts(long_truth$value, start=c(start_period, 1), end=c(end_period + 2, 56), frequency=56)
...
extended_df |>
  ...
  dplyr::mutate(q2.5 = value, q25 = value, q50 = value, q75 = value, q97.5 = value) |>
  dplyr::select("date", "level", "q2.5": "q97.5") |>
  dplyr::bind_rows(reconcile_df_old) |>
  ggplot(aes(x = date, group = level)) +
  geom_line(data = extended_df, aes(x = date, y = value, group = level), col = 1) +
  geom_point(data = extended_df, aes(x = date, y = value, group = level), col = 1) +
  geom_ribbon(aes(ymin = q2.5, ymax = q97.5, fill = "95% PI"), alpha = .75) +
...
Overall, the Gaussian probabilistic method and THieF method tend to produce fairly similar results, though the intervals are a little narrower for the Gaussian probabilistic forecasts, especially as the number of samples increases. The non-parametric bootstrap method tends to be less accurate, which reflects the findings in the `FoReco` paper, and sometimes will produce strange results, especially when the number of samples is low.
