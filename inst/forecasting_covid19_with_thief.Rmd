...
wis_1wk_plot_US <- forecast_date_metrics_US %>%
  ...
  geom_line(aes(color=model), alpha = 0.8) +
  geom_point(aes(color=model), alpha = 0.8, size=0.75) +
...
The THieF method is a novel approach that combines hierarchical forecasting and temporal aggregation for time series. This approach has shown promising results in terms of accuracy improvements over conventional methods due to its use of forecast combination. I will explore this method as a new way to model incident hospitalizations to investigate whether its construction may result in more accurate forecasts. Notably, THieF's forecast combination, while different from that of existing COVID-19 ensembles, may indicate it could also show good performance like this type of ensemble.   
...
### Conclusion
There are many challenges that come with implementing forecasts in an infectious disease setting. The impact of human behavior on the path of the epidemic complicates the system being forecast and induces a forecasting feedback loop which causes certain unique difficulties. Data availability and quality can also complicate infectious disease forecasting in ways that it might not in another field.   

THieF's unique methodology and its proven accuracy gains suggest that it might have something to offer. First, THieF generates its base forecasts with an underlying phenomenological model, and these types of models have been shown to perform well when applied to short-term outbreak forecasting. Second, since THieF has not been applied in infectious disease forecasting previously but has shown improvements in other contexts, this method could offer insight about effective modeling techniques (and perform well) in an outbreak setting.

That being said, the structure of the THieF methodology results in certain limitations that may impact its application in its current form. The topmost level of the temporal hierarchy restricts the lower level aggregation levels to be factors of the top level's time scale, which could result in important seasonalities being excluded. This is especially problematic when only limited data is available. Further, THieF currently lacks a simple probabilistic implementation, presenting a not insignificant obstacle when applying the methodology to settings that necessitate probabilistic forecasts.   
