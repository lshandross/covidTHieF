...
fc_dates <- c(as.Date("2021-01-11") + weeks(4*(0:7)),as.Date("2021-01-10") + weeks(4*(0:7)))

fc_ver <- fc_test %>% rbind(forecasts_baseline) 
for (i in 1:3) {
  fdat <- fc_ver %>%
...
scores_df <- score_forecasts(forecasts=fc_test, return_format="wide", truth=full_hosp_truth, use_median_as_point=TRUE)
scores_list <- list()
for (i in 1:6) {
  scores_list[[i]] <- 
    score_forecasts(
