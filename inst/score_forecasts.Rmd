...
horizon_summary <- forecasts_baseline %>%
  group_by(model, location, horizon) %>%
  summarize(rows = n())
  
horizon_wk_summary <- forecasts_baseline %>%
  mutate(
    horizon_wk = case_when(
      horizon %in% c(1:7) ~ 1,
      horizon %in% c(8:14) ~ 2,
  ...
  group_by(model, location, horizon_wk) %>%
  summarize(rows = n())

horizon_scores <- score_baseline %>%
...
#   select(1:16,18)
# 
# forecasts_baseline <- forecasts_baseline %>%
#   mutate(horizon_wk=ceiling(as.numeric(target_end_date-forecast_date)/7)) %>%
#   filter(horizon_wk %in% 1:4, 
#          target_end_date <= as.Date("2021-07-05"))

#save(forecasts_ver, scores, file="../data/versioned_fc_df.RData")
```
...
for (i in 1:3) {
  ...
  p <- plot_forecasts(fdat, 
                      #models = c("COVIDhub-baseline", models[2]),  
...
#   
#   p <- plot_forecasts(fdat, 
#                       models = c("COVIDhub-baseline", models[2], models[4], models[6]),  
# #                      truth_data = filter(full_hosp_truth, target_end_date <= as.Date("2021-05-03")),
#                       target_variable = "inc hosp", 
#                       intervals = c(.5, .95), 
#                       truth_source = "HealthData",
...
scores_df <- score_forecasts(forecasts=forecasts_ver, return_format="wide", truth=full_hosp_truth, use_median_as_point=TRUE)
...
for (i in 1:6) {
  scores_list[[i]] <- 
    score_forecasts(
      forecasts=filter(forecasts_df, model==models[i]),
      return_format="wide",
...
horizon_wk_metrics_states <- scores %>%
  rbind(score_baseline) %>%
  filter(target_end_date <= as.Date("2021-07-05"),
         forecast_date >= as.Date("2021-01-11") & location == "22",
         location != "US") %>%
  group_by(model, horizon_wk) %>%
  summarize(
    wis = mean(wis), mae=mean(abs_error),
...
```{r}  
# Fix issue with no baseline model showing up
