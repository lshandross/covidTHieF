...
models <- c("Base_arima_4root", "Multiple3_arima_4root", "Topmost8_arima_4root")
...
forecasts_df <- rbind(forecasts2, forecasts3)

for (j in 1:31) {
  if(j != 1) {
    df <- thief_fc_full[[j]] %>%
      mutate(model = "Topmost8_arima_noTrans")
    forecasts <- rbind(forecasts, df)
  } else {
    forecasts <- thief_fc_full[[j]] %>%
      mutate(model = "Topmost8_arima_noTrans")
  }
}
...
for (i in 1:3) {
  fdat <-forecasts_df %>% filter(location == locs[i], forecast_date %in% fc_dates)
...
```{r}
scores2 <- score_forecasts(forecasts=forecasts2, return_format="wide", truth=full_hosp_truth, use_median_as_point=TRUE)
scores_df <- rbind(scores, scores2)
#getwd()
save(forecasts_df, scores_df, file="../data/loadedfc_scores_df.RData")
