...
ht_day_ts_ <- ts(pull(hosp_truth, "value"), start = c(1,1), end = c(7, 29), frequency = 56) 
...
plot_thief <- 
  function(base_forecasts, reconciled_forecasts, ts_dates, extended_truth) {
    ...
    { # This code is not optimized for plotting when there are many levels
      ...
      axis(1, at=4:(periods + 6), labels = ts_dates) # changes axis labels to provided dates
      axis(2)
    }
...
```{r}
# Lower Forecasts
low_fc <- as_tibble(temp_reconciled[[1]][["lower"]]) # Change TS object to tibble
...
# Higher Forecasts
high_fc <- as_tibble(temp_reconciled[[1]][[6]]) # Change TS object to tibble
...
# Point Forecasts
point_fc <- as_tibble(temp_reconciled[[1]][[4]]) %>% # Change TS object to tibble
...
temp_rec_df <- transform_to_hub_df(temp_reconciled, "model", "US")

thief_wrapper <- 
  function(df, ts_col = "value", start_date, end_date, fips_code, aggregate_levels, frequency, pi_levels, model_name) {
    # If df == NULL, stop "you have not provided a data frame"
    # if df is not a a data frames, stop "Please provide a a data frames"
    ts_col <- ts_col
    ...
    plot_thief_agg(thief_aggregation, start_date) # use if statement to turn on?
    ...
    extended_agg <- extended_truth_data(df, ts_col, start_date, end_date, fips_code, aggregate_levels, frequency) # use if statement to turn on?
    ts_dates <- get_ts_dates(start_date, end_date, frequency = 56) # use if statement to turn on?
    plot_thief(base_fc, reconciled_fc, ts_dates, extended_agg) # use if statement to turn on?
...
covid_thief <- 
  function(df, ts_col, start_date, end_date, fips_vec, aggregate_levels, frequency, pi_levels, model_name) {
    all_locs_df <- map_dfr(
      .x = fips_vec,
      .f = function(fips_code) {
        thief_wrapper(df, ts_col, start_date, end_date, fips_code, aggregate_levels, frequency, pi_levels, model_name) 
      }
    )
    return(all_locs_df)
  }
