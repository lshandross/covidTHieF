...
plot_thief <- 
  function(base_forecasts, reconciled_forecasts, actual_dates) {
    par(mfrow=c(ceiling(length(base_forecasts)),2), mai=c(0.35,0.5,0.35,0.35))
    ...
    { # This code is not optimized for plotting when there are many levels
      ...
      lines(long_aggs[[i]], col='black', lwd=1.5, lty = "dotted")
      axis(1, at=1:(periods + 1), labels = dates_actual_forecast)
      axis(2)
    }
...
extended_truth_data <- 
  function(df, ts_col = "value", start_date, end_date, fips_code, aggregate_levels = list(56, 28, 14, 7, 1), frequency = 56) {
    # an extra 1*(top agg level) of truth data to plot against forecasts
    freq <- frequency
    end_date <- end_date + weeks (freq)
    aggregate_thief_df(df, ts_col, start_date, end_date, fips_code, aggregate_levels, frequency) 
