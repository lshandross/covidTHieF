...
#base_fc_day; base_fc_day2
# Compute base forecasts
be <- list(); fits <- list()
for(i in seq_along(test)){
  be[[i]] <- forecast(auto.arima(test[[i]]), 
                               h=frequency(test[[i]]), 
                               level = pi_levels)
}

#reconcilethief(be, aggregatelist = agg_list) # produces a warning
```
...
transform_to_hub_df(temp_reconciled, "model", "US")
```
