...
# Export our function on the cluster
clusterExport(cl, list('generate_thief_wk', 'mon_fc_dates', 'training_truth_df'))
#clusterExport(cl, list('generate_thief_wk', 'mon_fc_dates'))

# Run function across previously specified number of cores
system.time({
  thief_fc_full <- c(parLapply(cl, mon_fc_dates[20:25], fun = generate_thief_wk))
})
...
#modfc_12wk_4root <- c()
for (i in 1:2) {
  write_csv(thief_fc_full[[i]][[1]], file=paste("data/", models[2], "/", mon_fc_dates[i+17], "-", models[2], ".csv", sep=""))
#  modfc_4wk_4root <- rbind(modfc_12wk_4root, thief_fc_full[[i]][[2]])
  modfc_4wk_noTrans <- rbind(modfc_4wk_noTrans, thief_fc_full[[i]][[2]])
}

save(modfc_4wk_noTrans, file=paste("data/", models[2], "/", models[2], ".RData", sep=""))
