...
#'   of type \code{char}.
#' @param frequency Integer seasonal period. Defaults to a value of 1,
#'   indicating no seasonality.
...
#'
#' @return A time series \code{ts} object made by coercing the original
#'   truth data into a time series format.
#' @export
#'
...
#'
#' @param df A data frame containing the desired truth data as one of the columns.
#'   Used to create the forecasts. Defaults to NULL in which hospitalization
#'   truth data is sourced as of the user-specified \code{end_date}.
#' @param ts_col The name of the column containing the truth data. This column is
#'   coerced into a time series object of class \code{ts} and thus should be a 
#'   numeric type.
#' @param start_date A date from which the truth data begins.
#' @param end_date A date where the truth data ends and the forecasts begin. If
#'   \code{df=NULL}, also specifies the date from which the hospitalization
#'   truth data sourced.
...
#' @param pi_levels A vector of prediction interval levels to calculate.
#' @param plot.ts \code{logical} that specifies whether a plot of the original
#'   time series should be generated.
#' @param plot.forecasts \code{logical} that specifies whether a plot of the
#'   forecasts should be generated.
#' @param transform.4root \code{logical} that specifies whether a variance
#'   stabilizing fourth root transformation should be performed on the data.
#'   (This data transformation is undone after all of the forecasts are re-formatted
#'   into a data frame.)
#'
#' @return A list containing two items: a data frame containing COVID-19 incident
#' hospitalization forecasts with a US COVID-19 Forecast Hub format and a data frame
#' containing the base forecast object with other relevant identifying information.
#' @export
#'
#' @importFrom rlang .data
...
sarima_wrapper <-
  function(df = NULL, ts_col = "value", start_date, end_date, fips_code, frequency = 1, pi_levels, plot.forecasts = TRUE, transform.4root = FALSE) {
    ...
    time_series <- suppressWarnings(
      get_truth_ts(df, ts_col, start_date, most_recent_date, fips_code, frequency, transform.4root)
    )

    # if (plot.ts == TRUE) plot_thief_agg(thief_aggregation, start_date)

    forecasts <- forecast::forecast(forecast::auto.arima(time_series), h = h_ahead, level = pi_levels)

    if (plot.forecasts == TRUE) {
      extended_truth <- suppressWarnings(
        get_truth_ts(df, ts_col, start_date, end_date + lubridate::days(h_ahead), fips_code, frequency)
      )
      ts_dates <- get_ts_dates(start_date, most_recent_date + lubridate::days(h_ahead), frequency)
      plot(forecasts, shadecols = c("light gray", "#EEC2C2", "#900000"),
           xaxt = "n", #axes = FALSE,
           ylim = c(0, max(forecasts$x, forecasts$upper)))
      graphics::lines(forecasts$mean, col = "red", lwd = 2) # plots red base forecasts line
      if(!is.null(extended_truth)) {
        # plots extended truth data against forecasts
        graphics::lines(extended_truth, col = 'black', lwd = 1.5, lty = "dotted")
      }
      graphics::axis(1, at = 1:periods_extended + 1, labels = ts_dates) # changes axis labels to provided dates
      graphics::axis(2)
    }

    hub_df <- forecasts |>
      transform_to_hub_df(most_recent_date, fips_code, pi_levels, h_ahead, transform.4root)
    model_info <- dplyr::tibble(
      forecast_date = end_date, location = fips_code,
      level = frequency,
      fc_obj = list(forecasts)
    )
