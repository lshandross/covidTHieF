...
#' @param plot.aggregates \code{logical} that specifies whether a plot of the
#'   aggregated original time series should be generated.
#' @param plot.forecasts \code{logical} that specifies whether a collection of
#'   plots of the forecasts should be generated.
...
#'   incident hospitalization forecasts with a US COVID-19 Forecast Hub format
#'   and a data frame containing the base forecast object and the reconciled
#'   forecast object with other relevant identifying information.
#' @export
#'
#' @importFrom rlang .data
thief_wrapper <-
  function(df = NULL, ts_col = "value", start_date, end_date, fips_code,
           aggregate_levels, frequency, pi_levels, plot.aggregates = TRUE,
           plot.forecasts = TRUE, transform.4root = FALSE) {
    ...
    reconciled_fc <- base_fc |>
      thief::reconcilethief(aggregatelist = aggregate_levels)

    if (plot.forecasts == TRUE) {
      extended_agg <- df |>
        extended_truth_data(start_date, most_recent_date, fips_code,
                            aggregate_levels, frequency)
      ts_dates <- get_ts_dates(start_date, most_recent_date, frequency)
      plot_thief(base_fc, reconciled_fc, ts_dates, extended_agg, NULL)
    }

    hub_df <- reconciled_fc |>
      transform_to_hub_df(most_recent_date, fips_code, pi_levels,
                          h_ahead, transform.4root)
    model_info <- dplyr::tibble(
      forecast_date = end_date, location = fips_code,
      level = aggregate_levels,
