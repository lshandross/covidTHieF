...
#' @importFrom magrittr %>%
covid_thief <-
  function(df = NULL, ts_col = "value", start_date, end_date, fips_vec,
           aggregate_levels, frequency, pi_levels, transform.4root = FALSE) {
    if (is.null(df)) {
    ...
    most_recent_date <- df %>%
      dplyr::slice_max(.data[["target_end_date"]], n = 1, with_ties = FALSE) %>%
      dplyr::pull(.data[["target_end_date"]])
    ...
    all_locs_list <- purrr::map(
      ...
      .f = function(fips_code) {
        suppressWarnings(
          thief_wrapper(df = NULL, ts_col, start_date, end_date, fips_code,
                        aggregate_levels, frequency, pi_levels,
