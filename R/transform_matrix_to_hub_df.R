...
#'   quartiles.
#' @param h_ahead Number specifying the time units ahead of the longest horizon
...
transform_matrix_to_hub_df <-
  function(fc_matrix, forecast_date, fips_code, quantile_levels = c(0.25, 0.5, 0.75), h_ahead = 56) {
    ...
    fc_quantiles |>
      ...
      dplyr::filter(
        .data[["k"]] == 1, # keep only daily forecasts
        .data[["horizon"]] <= h_ahead
      ) |>
